...
7. Install the **istio-init** Helm chart to bootstrap all the Istio’s [Custom Resource Definitions](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/#customresourcedefinitions)(CRDs)
...
-  To configure **Istio metrics** to collect **telemetry** from the mesh (so it can can later be accessed with Prometheus) run the following command:
